...
def unsharp_mask_spatial(img, kdim_x, kdim_y, count, L_max, L_min):
    """
    求原影像在Spatial domain上經unsharp masking後的結果。
    ...
    sharp_img = img_addition(unsharp_mask, gray_img, 1, L_max, L_min)

    title = 'image after unsharp masking \nin spatial domain'
    
    disp_img(sharp_img, 1, count, title)
    
    
def unsharp_mask_frequency(img, radius, count, L_max, L_min):
    """
    求原影像在frequency domain上經unsharp masking後的結果。
    ...
    disp_img(sharp_img, 2, count, title)
...
if __name__ == '__main__':
    for files in glob.glob(path + '*'):
        ...
        unsharp_mask_spatial(gray_img, 3, 3, count, L_max, L_min)
        
        unsharp_mask_frequency(gray_img, radius, count, L_max, L_min)
