...
variances_test = [0.05, 0.2, 5]

path = 'HW4_test_image/'

fig, axes = plt.subplots(1 + len(variances_test),
...
if __name__ == '__main__':
    for files in glob.glob(path + '*.jpg'):
        ...
        for test in range(1, len(variances_test) + 1):
        
            blur_img = blurring(img, kernel_size, variances_test[test - 1] , L_min, L_max) 
            ...
            disp_img(test, count, 'Edges of image with varience = ' + str(variances_test[test - 1]), edge)
