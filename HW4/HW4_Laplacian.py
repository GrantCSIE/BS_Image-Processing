# -*- coding: utf-8 -*-
"""
Created on Sun Dec 17 15:05:59 2023
...
def elementwise_multi(src_img, kernel, L_min, L_max):
    ...
    for i in range(dim_x):
        for j in range(dim_y):
            transformed_img[i][j] = np.sum(padded_image[i:i + kdim_x, j:j + kdim_y] * kernel)
...
if __name__ == '__main__':
    for files in glob.glob(path + '*.jpg'):
        ...
        blur_img = blurring(img, kernel_size, variance, L_min, L_max)
